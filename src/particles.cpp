...
private:
    ...
    VkPhysicalDevice physicalDevice = VK_NULL_HANDLE;   // implicitly cleaned up
    VkDevice device;
    ...
    std::vector<VkCommandBuffer> commandBuffers;      // implicitly cleaned up
...
#pragma region main-function
    ...
    void createLogicalDevice() {
        ...
        if (enableValidationLayers) {   
            // better write it, although vulkan has no distinction between instance and device specific validation layers.
            createInfo.enabledLayerCount = static_cast<uint32_t>(validationLayers.size());
            createInfo.ppEnabledLayerNames = validationLayers.data();
        } else {
            createInfo.enabledLayerCount = 0;
    ...
    void createComputePipeline() {
        auto computeShaderCode = readFile("shaders/particles/compute.spv");
    ...
    void createFramebuffers() {
        ...
        for (size_t i = 0; i < swapChainImageViews.size(); ++i) {
            std::array<VkImageView, 1> attachments = {
                ...
                // only a single subpass is running at the same time due to semaphores.
                // so colorImageView and depthImageView is shared.
            };
    ...
    void createShaderStorageBuffers() {
        ...
        for (auto& particle : particles) {
            ...
            float theta = rndDist(rndEngine) * 2 * 3.1415927f;
...
#pragma region detailed-function
    ...
    VkBuffer& buffer, VkDeviceMemory& bufferMemory) {
        ...
        bufferInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE; // can be owned by a specific queue or shared between multiple, EXCLUSIVE means used by specific
    ...
    void generateMipmaps(VkImage image, VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels) {
        ...
        for (uint32_t i = 1; i < mipLevels; ++i) {
            ...
            // 将blit完的src mip-image转化为shader可读的形式
...
#pragma endregion detailed-function
